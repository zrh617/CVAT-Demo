...
import { DimensionType } from '@/core-wrapper';
import {
    CombinedState,
    FrameSpeed,
    Workspace,
...
function mapStateToProps(state: CombinedState): any {
    ...
    let frameIsDeleted: boolean = false
    state.annotation.player.frame.data && (frameIsDeleted = state.annotation.player.frame.data)
...
class AnnotationTopBarContainer extends React.PureComponent<Props, State> {
    ...
    public componentDidMount(): void {
        const {
            autoSaveInterval, router, jobInstance, setForceExitAnnotationFlag,
        } = this.props;
        this.autoSaveInterval = window.setInterval(this.autoSave.bind(this), autoSaveInterval);

        // eslint-disable-next-line @typescript-eslint/no-this-alias
        const self = this;
        // this.unblock = router.block((location: any) => {
