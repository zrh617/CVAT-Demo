...
export default class Webhook {
    ...
    public toJSON(): RawWebhookData {
        ...
        if (Number.isInteger(this.organizationID)) {
            result.organization_id = this.organizationID || 0;
        }

        if (Number.isInteger(this.projectID)) {
            result.project_id = this.projectID || 0;
        }
    ...
    public async save(): Promise<Webhook> {
        const result: any = await PluginRegistry.apiWrapper.call(this, Webhook.prototype.save as any);
        return result;
    }

    public async delete(): Promise<void> {
        const result = await PluginRegistry.apiWrapper.call(this, Webhook.prototype.delete as any);
        return result;
    }

    public async ping(): Promise<void> {
        const result = await PluginRegistry.apiWrapper.call(this, Webhook.prototype.ping as any);
        return result;
    }
}
