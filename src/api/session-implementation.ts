...
async function deleteFrameWrapper(this: any, jobID, frame) {
...
async function restoreFrameWrapper(this: any, jobID, frame) {
    const history = getHistory(this);
    const redo = async () => {
        restoreFrame(jobID, frame);
    };
    ...
    Task.prototype.save.implementation = async function (onUpdate) {
        if (typeof this.id !== 'undefined') {
            ...
            await Promise.all((taskData.labels || []).map((label: Label): Promise<unknown> => {
                if (label.deleted) {
                    return serverProxy.labels.delete(label.id || 0);
                }

                if (label.patched) {
                    return serverProxy.labels.update(label.id || 0, label.toJSON());
                }
            ...
            taskData.labels = (taskData.labels || [])
                .filter((label: any) => !Number.isInteger(label.id)).map((el) => el.toJSON());
