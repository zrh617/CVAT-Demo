...
class Annotation {
    ...
    public serverID: number | null | undefined;
    ...
    protected saveLabel(label: Label, frame: number): void {
        const undoLabel: any = this.label;
        const redoLabel: any = label;
    ...
    protected appendDefaultAttributes(label: Label): void {
        const labelAttributes: any = label.attributes;
...
class Drawn extends Annotation {
    ...
    public shapeType: ShapeType | any;
    ...
    protected validateStateBeforeSave(data: ObjectState, updated: ObjectState['updateFlags'], frame?: any): number[] {
...
export class Shape extends Drawn {
    ...
    public toJSON(): RawShapeData {
        const result: RawShapeData = {
            ...
            attributes: Object.keys(this.attributes).reduce((attributeAccumulator: any, attrId) => {
    ...
    public save(frame: number, data: ObjectState): ObjectState {
        ...
        const fittedPoints: any = this.validateStateBeforeSave(data, updated, frame);
        const { rotation } = data;
...
export class Track extends Drawn {
    ...
    public toJSON(): RawTrackData {
        ...
        const result: RawTrackData = {
            ...
            shapes: Object.keys(this.shapes).reduce((shapesAccumulator: any, frame) => {
                shapesAccumulator.push({
                    ...
                    attributes: Object.keys(this.shapes[frame].attributes).reduce(
                        (attributeAccumulator: any, attrId) => {
                            if (labelAttributes[attrId].mutable) {
                                attributeAccumulator.push({
                                    spec_id: +attrId,
    ...
    protected getPosition(
        targetFrame: number, leftKeyframe: number | any, rightFrame: number | any,
    ): InterpolatedPosition & { keyframe: boolean } {
        const leftFrame = targetFrame in this.shapes ? targetFrame : leftKeyframe;
        const rightPosition = Number.isInteger(rightFrame) ? this.shapes[rightFrame || 0] : null;
        const leftPosition = Number.isInteger(leftFrame) ? this.shapes[leftFrame || 0] : null;
        ...
        const singlePosition: any = leftPosition || rightPosition;
        if (singlePosition) {
            return {
...
export class RectangleShape extends Shape {
    ...
    static distance(points: number[], x: number, y: number, angle: number): any {
...
export class PolygonShape extends PolyShape {
    ...
    static distance(points: number[], x: number, y: number): any {
...
export class CuboidShape extends Shape {
        ...
        function makeHullPresorted(points: Point2D[]): Point2D[] {
            ...
            for (let i = 0; i < points.length; i += 1) {
                ...
                while (upperHull.length >= 2) {
                    const q: any = upperHull[upperHull.length - 1];
                    const r: any = upperHull[upperHull.length - 2];
            ...
            for (let i = points.length - 1; i >= 0; i -= 1) {
                ...
                while (lowerHull.length >= 2) {
                    const q: any = lowerHull[lowerHull.length - 1];
                    const r: any = lowerHull[lowerHull.length - 2];
    ...
    static distance(actualPoints: number[], x: number, y: number): any {
        const points: any[] = [];
...
export class SkeletonShape extends Shape {
    ...
    public updateServerID(body: RawShapeData): void {
        ...
        for (const element of body.elements) {
            const thisElement: any = this.elements.find((_element: Shape) => _element.label.id === element.label_id);
    ...
    public save(frame: number, data: ObjectState): ObjectState {
        ...
        const updateElements = (affectedElements, action, property: 'points' | 'occluded' | 'hidden' | 'lock') => {
            const undoSkeletonProperties: any = this.elements.map((element) => element[property]);
            const undoSource = this.source;
            const redoSource = this.readOnlyFields.includes('source') ? this.source : Source.MANUAL;
            ...
            const redoSkeletonProperties: any = this.elements.map((element) => element[property]);
...
export class MaskShape extends Shape {
    ...
    //     Annotation.prototype.validateStateBeforeSave.call(this, data, updated);
    //     if (updated.points) {
    //         const { width, height } = this.frameMeta[frame || 0];
...
MaskShape.prototype.toJSON = function (this: any) {
    const result: any = Shape.prototype.toJSON.call(this);
    result.points = this.points.slice();
    result.points.push(this.left, this.top, this.right, this.bottom);
    return result;
};

MaskShape.prototype.get = function (this: any, frame) {
...
export class RectangleTrack extends Track {
    constructor(data: RawTrackData, clientID: number, color: string, injection: AnnotationInjection) {
        ...
        this.shapeType = ShapeType.RECTANGLE;
        this.pinned = false;
        for (const shape of Object.values(this.shapes) as any) {
...
class PolyTrack extends Track {
    ...
    protected interpolatePosition(leftPosition, rightPosition, offset): InterpolatedPosition {
        ...
        function toArray(points: { x: number; y: number; }[]): number[] {
            return points.reduce((acc: any, val) => {
        ...
        const interpolatedPoints = Object.keys(completedMatching)
            ...
            .sort((a, b) => a - b)
            .reduce((acc: any[], leftPointIdx) => {
                const leftPoint = leftPoints[leftPointIdx];
                for (const rightPointIdx of completedMatching[leftPointIdx] as any) {
...
export class SkeletonTrack extends Track {
    ...
    constructor(data: RawTrackData, clientID: number, color: string, injection: AnnotationInjection) {
        ...
        this.pinned = false;
        this.elements = data.elements?.map((element: any) => (
            /* eslint-disable-next-line @typescript-eslint/no-use-before-define */
            trackFactory({
                ...element,
                group: this.group,
        ...
        )).sort((a: any, b: any) => a.label.id - b.label.id) as any as Track[];
    }

    public updateServerID(body: any): void {
        Track.prototype.updateServerID.call(this, body);
        for (const element of body.elements) {
            const thisElement: any = this.elements.find((_element: Track) => _element.label.id === element.label_id);
            thisElement.updateServerID(element);
        }
    }
    ...
    public save(frame: number, data: ObjectState): ObjectState {
        ...
        const updateElements = (affectedElements, action, property: 'hidden' | 'lock' | null = null): void => {
            const undoSkeletonProperties = this.elements.map((element) => element[property || 0] || null);
            const undoSkeletonShapes = this.elements.map((element) => element.shapes[frame]);
            const undoSource = this.source;
            const redoSource = this.readOnlyFields.includes('source') ? this.source : Source.MANUAL;
            ...
            const redoSkeletonProperties = this.elements.map((element) => element[property || 0] || null);
            const redoSkeletonShapes = this.elements.map((element) => element.shapes[frame]);
    ...
    protected getPosition(
        ...
        const leftFrame = targetFrame in this.shapes ? targetFrame : leftKeyframe;
        const rightPosition = Number.isInteger(rightKeyframe) ? this.shapes[rightKeyframe || 0] : null;
        const leftPosition = Number.isInteger(leftFrame) ? this.shapes[leftFrame || 0] : null;
...
export function trackFactory(trackData: RawTrackData, clientID: number, injection: AnnotationInjection): any {
    if (trackData.shapes.length) {
        const { type } = trackData.shapes[0];
        const color = colors[clientID % colors.length];

        let trackModel: any = null;
        switch (type) {
            case ShapeType.RECTANGLE:
