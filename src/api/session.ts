...
export class Job extends Session {
    ...
    constructor(initialData) {
        ...
        const updateTrigger: any = new FieldUpdateTrigger();
...
export class Task extends Session {
    ...
    constructor(initialData) {
        ...
        if (Array.isArray(initialData.segments)) {
            for (const job of initialData.segments) {
                const jobInstance = new Job({
                    url: job?.jobs[0]?.url,
                    id: job?.jobs[0]?.id,
                    assignee: job?.jobs[0]?.assignee,
                    state: job?.jobs[0]?.state,
                    stage: job?.jobs[0]?.stage,
