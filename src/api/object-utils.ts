...
export function checkNumberOfPoints(shapeType: ShapeType | null, points: number[]): void {
...
export function attrsAsAnObject(attributes: any): Record<number, Attribute> {
    return attributes.reduce((accumulator, value: any) => {
...
export function checkShapeArea(shapeType: ShapeType | null, points: number[]): boolean {
...
export function mask2Rle(mask: number[]): number[] {
    return mask.reduce((acc: any[], val, idx, arr) => {
        if (idx > 0) {
            if (arr[idx - 1] === val) {
                acc[acc.length - 1] += 1;
            } else {
