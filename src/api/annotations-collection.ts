...
export default class Collection {
    ...
    get(frame: number, allTracks: boolean, filters: string[]): ObjectState[] {
        ...
        const objects: any[] = [].concat(tracks as any, shapes as any, tags as any);
        const visible: any[] = [];
        ...
        visible.forEach((stateData) => {
            if (!filters.length || filtered.includes(stateData.clientID)) {
                const objectState: any = new ObjectState(stateData);
    ...
    _mergeInternal(objectsForMerge: (Track | Shape)[], shapeType: ShapeType, label: Label): any {
        const keyframes: any = {}; // frame: position
        ...
        const labelAttributes: any = label.attributes?.reduce((accumulator, attribute) => {
            accumulator[attribute.id || 0] = attribute;
            return accumulator;
        }, {});

        for (let i = 0; i < objectsForMerge.length; i++) {
            // For each state get corresponding object
            const object: any = objectsForMerge[i];
            ...
            if (object instanceof Shape) {
                ...
                keyframes[object.frame] = {
                    ...
                    outside: false,
                    attributes: Object.keys(object.attributes).reduce((accumulator: any, attrID) => {
                ...
                for (const keyframe of Object.keys(trackShapes)) {
                    ...
                    keyframes[keyframe] = {
                        ...
                        attributes: updatedAttributes ? Object.keys(attributes).reduce((accumulator: any, attrID) => {
                            accumulator.push({
                                spec_id: +attrID,
            ...
            if (object.shapeType === ShapeType.SKELETON) {
                for (const element of (object as any).elements) {
                    // for each track/shape element get its first objectState and keep it
                    elements[element.label.id || 0] = [
                        ...(elements[element.label.id || 0] || []), element,
                    ];
                }
            }
        }
        ...
        if (shapeType === ShapeType.SKELETON) {
            for (const sublabel of label?.structure?.sublabels as any) {
        ...
        const track = {
            ...
            label_id: label.id,
            attributes: Object.keys(objectsForMerge[0].attributes).reduce((accumulator: any, attrID) => {
    ...
    merge(objectStates: any[]): void {
        ...
        const objectsForMerge = objectStates.map((state) => {
            ...
            const object = this.objects[state.clientID || 0];
    ...
    _splitInternal(objectState: ObjectState, object: Track, frame: number): RawTrackData[] {
        const labelAttributes: any = object.label.attributes?.reduce((accumulator, attribute) => {
            accumulator[attribute.id || 0] = attribute;
            return accumulator;
        }, {});
        ...
        const position = {
            ...
            attributes: Object.keys(objectState.attributes).reduce((accumulator: any, attrID) => {
                if (labelAttributes[attrID].mutable) {
                    accumulator.push({
                        spec_id: +attrID,
    ...
    split(objectState: ObjectState, frame: number): void {
        ...
        const object = this.objects[objectState.clientID || 0] as Track;
        if (typeof object === 'undefined') {
    ...
    statistics(): Statistics {
        ...
        for (const object of Object.values(this.objects)) {
            ...
            if (objectType === 'tag') {
                labels[label || 0].tag++;
                labels[label || 0].manually++;
                labels[label || 0].total++;
            } else if (objectType === 'track') {
                scanTrack(object);
            } else {
                const { shapeType } = object as Shape;
                labels[label || 0][shapeType].shape++;
                labels[label || 0].manually++;
                labels[label || 0].total++;
                if (shapeType === ShapeType.SKELETON) {
    ...
    put(objectStates: ObjectState[]): number[] {
        ...
        function convertAttributes(this: any, accumulator, attrID) {
            const specID = +attrID;
        ...
        for (const state of objectStates) {
            ...
            const labelAttributes: any = state.label.attributes?.reduce((accumulator, attribute) => {
                accumulator[attribute.id || 0] = attribute;
                return accumulator;
            }, {});
                ...
                if (state.objectType === 'shape') {
                    ...
                    constructed.tracks.push({
                        ...
                        elements: state.shapeType === 'skeleton' ? state.elements.map((element) => {
                            ...
                            const elementAttributes: any = element.label.attributes?.reduce((accumulator, attribute) => {
                                accumulator[attribute.id || 0] = attribute;
                                return accumulator;
                            }, {});
        ...
        for (const object of importedArray as any) {
    ...
    select(objectStates: ObjectState[], x: number, y: number): {
        ...
        for (const state of objectStates) {
            ...
            const object: any = this.objects[state.clientID || 0];
            if (typeof object === 'undefined') {
                throw new ArgumentError('The object has not been saved yet. Call annotations.put([state]) before');
            }
    ...
    searchEmpty(frameFrom: number, frameTo: number): number | null {
        ...
        for (let frame = frameFrom; predicate(frame); frame = update(frame)) {
            ...
            for (const track of filteredTracks) {
                const keyframes: any = track.boundedKeyframes(frame);
                const { prev, first } = keyframes;
                const last = prev === null ? first : prev;
                const lastShape = track.shapes[last || 0];
    ...
    search(filters: string[], frameFrom: number, frameTo: number): number | null {
        ...
        for (let frame = frameFrom; predicate(frame); frame = update(frame)) {
            ...
            const statesData: any = [].concat(
                (frame in this.shapes ? this.shapes[frame] as any : [])
                    .filter((shape) => !shape.removed)
                    .map((shape) => shape.get(frame)),
                (frame in this.tags ? this.tags[frame] as any : [])
                    .filter((tag) => !tag.removed)
