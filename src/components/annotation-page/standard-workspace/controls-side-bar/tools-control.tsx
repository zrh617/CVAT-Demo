...
import { Canvas, convertShapesForInteractor } from '@/canvas-wrapper';
import {
...
export class ToolsControlComponent extends React.PureComponent<Props, State> {
    ...
    private setActiveInteractor = (value: string): void => {
        ...
        const [interactor] = interactors.filter((_interactor: MLModel) => _interactor.id === value) as any;
            ...
            const trackingData = trackedShapes.reduce<AccumulatorType>(
                (acc: AccumulatorType, trackedShape: TrackedShape): AccumulatorType => {
                    ...
                    if (clientState && !clientState.outside) {
                        ...
                        if (stateIsRelevant) {
                            const container = acc.statefull[trackerModel.id || 0] || {
                                clientIDs: [],
                                shapes: [],
                                states: [],
                            };
                            ...
                            acc.statefull[trackerModel.id || 0] = container;
                        } else {
                            const container = acc.stateless[trackerModel.id || 0] || {
                                clientIDs: [],
                                shapes: [],
                            };
                            container.clientIDs.push(clientID);
                            container.shapes.push(points);
                            acc.stateless[trackerModel.id || 0] = container;
                        }
                    }
    ...
    private renderDetectorBlock(): JSX.Element {
        ...
        return (
            <DetectorRunner
                ...
                runInference={async (model: any, body: DetectorRequestBody) => {
                    try {
                                    ...
                                    attributes: (data.attributes as { name: string, value: string }[])
                                        .reduce((acc, attr) => {
                                            const [modelAttr] = Object.entries(body.mapping[modelLabel].attributes)
                                                .find((value: any[]) => value[1] === attr.name) || [];
