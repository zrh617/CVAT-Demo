...
import {
    useNavigate, matchRoutes, useLocation 
} from 'react-router';
...
function WebhooksPage(): JSX.Element | null {
    ...
    const projectsMatch: any = matchRoutes<any>([{ path: '/projects/:id/webhooks' }], location);
    ...
    useEffect(() => {
        if (projectsMatch && projectsMatch.params.id) {
            ...
            dispatch(getWebhooksAsync({ ...updatedQuery, projectId: +id }) as any);
        } else if (organization) {
            dispatch(getWebhooksAsync(updatedQuery) as any);
        } else {
            history.push('/');
        }
    }, [organization]);
