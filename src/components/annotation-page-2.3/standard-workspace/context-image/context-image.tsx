...
import { hideShowContextImage, getContextImageAsync } from '@/actions/annotation-actions-2.3';
...
function ContextImage(): JSX.Element | null {
    ...
    useSelector((state: CombinedState) => console.log(state))
    const { number: frame, hasRelatedContext } = useSelector((state: CombinedState) => state.annotation.player.frame);
    // const { hidden: contextImageHidden, fetching: contextImageFetching } = useSelector(
    //     (state: CombinedState) => state.annotation.player.contextImage,
    // );
    let contextImageData = useSelector(
        (state: CombinedState) => state.annotation.player.contextImage,
    )?.data;
    let contextImageHidden = useSelector(
        (state: CombinedState) => state.annotation.player.contextImage,
    )?.hidden;
    let contextImageFetching = useSelector(
        (state: CombinedState) => state.annotation.player.contextImage,
    )?.fetching;
    ...
    useEffect(() => {
        if (hasRelatedContext && !contextImageHidden && !requested) {
            dispatch(getContextImageAsync() as any);
            setRequested(true);
        }
