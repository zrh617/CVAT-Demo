...
import { AntdWidgets, Config, ImmutableTree, JsonLogicTree } from '@react-awesome-query-builder/antd';
...
import { changeAnnotationsFilters, fetchAnnotationsAsync, showFilters } from '@/actions/annotation-actions-2.3';
    ...
    useEffect(() => {
        if (visible) {
            const treeFromActiveFilters = activeFilters.length ?
                QbUtils.checkTree(QbUtils.loadFromJsonLogic(activeFilters[0], config) as any, config) :
                null;
            setState({
        ...
        <Menu>
            {filters
                .map((filter: StoredFilter) => {
                    ...
                    const tree: any = QbUtils.loadFromJsonLogic(filter.logic, config);
