...
export class EditHandlerImpl implements EditHandler {
    ...
    private setupTrailingPoint(circle: SVG.Circle): void {
        ...
        circle.on('mouseenter', (): void => {
            circle.attr({
                'stroke-width': consts.POINTS_SELECTED_STROKE_WIDTH / (this.geometry?.scale || 1),
            });
        });

        circle.on('mouseleave', (): void => {
            circle.attr({
                'stroke-width': consts.POINTS_STROKE_WIDTH / (this.geometry?.scale || 1),
            });
        });
    ...
    public configurate(configuration: Configuration): void {
        ...
        this.intelligentCutEnabled = configuration.intelligentPolygonCrop || false;
    }
