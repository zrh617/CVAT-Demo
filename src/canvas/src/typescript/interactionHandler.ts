...
export class InteractionHandlerImpl implements InteractionHandler {
    ...
    private shouldRaiseEvent(ctrlKey: boolean): boolean {
        ...
        const minNegVerticesDefined = Number.isInteger(minNegVertices) && minNegVertices && minNegVertices >= 0;
        const minPosVerticesAchieved = !minPosVerticesDefined || minPosVertices && minPosVertices <= positiveShapes.length;
        ...
        return (enabled && !ctrlKey && minimumVerticesAchieved && shapesWereUpdated) || false;
    }
    ...
    private interactPoints(): void {
        const eventListener = (e: MouseEvent): void => {
            if ((e.button === 0 || (e.button === 2 && this.interactionData.minNegVertices && this.interactionData.minNegVertices >= 0)) && !e.altKey) {
                e.preventDefault();
    ...
    public constructor(
        ...
        this.onInteraction = (shapes: InteractionResult[] | null, shapesUpdated?: boolean, isDone?: boolean): void => {
            ...
            onInteraction(shapes, shapesUpdated, isDone, this.threshold ? this.thresholdRectSize / 2 : 0);
        };
        this.canvas = canvas;
        ...
        this.drawnIntermediateShape = null;
        this.controlPointsSize = configuration.controlPointsSize || 0;
        this.selectedShapeOpacity = configuration.selectedShapeOpacity || 0;
        this.cursorPosition = {
            x: 0,
            y: 0,
        };
    ...
    public transform(geometry: Geometry): void {
        ...
        for (const element of window.document.getElementsByClassName('cvat_canvas_interact_intermediate_shape_point') as any) {
