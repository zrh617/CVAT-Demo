...
export class CanvasViewImpl implements CanvasView, Listener {
    ...
    private translatePointsFromRotatedShape(
        shape: SVG.Shape, points: number[], cx: any = null, cy: any = null,
    ): number[] {
        ...
        try {
            ...
            const point: any = this.content.createSVGPoint();
            ...
            if (cx !== null && cy !== null) {
                shape.rotate(rotation || 0, cx, cy);
            } else {
                shape.rotate(rotation || 0);
            }
        }
    ...
    private transformCanvas(): void {
        ...
        for (const element of [
            ...window.document.getElementsByClassName('svg_select_points') as any,
            ...window.document.getElementsByClassName('svg_select_points_rot') as any,
            ...window.document.getElementsByClassName('svg_select_boundingRect') as any,
        ]) {
            element.setAttribute('stroke-width', `${consts.POINTS_STROKE_WIDTH / this.geometry.scale}`);
            element.setAttribute('r', `${this.configuration.controlPointsSize || 0 / this.geometry.scale}`);
        }

        for (const element of window.document.getElementsByClassName('cvat_canvas_poly_direction') as any) {
        ...
        for (const element of window.document.getElementsByClassName('cvat_canvas_selected_point') as any) {
        ...
        for (const key of Object.keys(this.svgShapes)) {
            ...
            if (object.type === 'circle') {
                object.attr('r', `${this.configuration.controlPointsSize || 0 / this.geometry.scale}`);
            }
        ...
        for (const skeletonEdge of window.document.getElementsByClassName('cvat_canvas_skeleton_edge') as any) {
    ...
    private hideDirection(shape: SVG.Polygon | SVG.PolyLine): void {
        ...
        const directions: any = node.getElementsByClassName('cvat_canvas_poly_direction');
    ...
    private selectize(value: boolean, shape: SVG.Element): void {
            ...
            if (this.activeElement.clientID !== null) {
                ...
                if (['polygon', 'polyline', 'points'].includes(state.shapeType)) {
                    if (state.shapeType === 'points' && (e.altKey || e.ctrlKey)) {
                        const selectedClientID = +((e.target as any).parentElement as any).getAttribute('clientID');
        ...
        if (value) {
            ...
            (shape as any).selectize(value, {
                deepSelect: true,
                pointSize: (2 * ( this.configuration.controlPointsSize || 0 )) / this.geometry.scale,
        ...
        const rotationPoint: any = window.document.getElementsByClassName('svg_select_points_rot');
        const topPoint = window.document.getElementsByClassName('svg_select_points_t');
        if (rotationPoint && !rotationPoint.children.length) {
            if (topPoint) {
                const rotY = +(rotationPoint as any).getAttribute('cy');
                const topY = +(topPoint as any).getAttribute('cy');
                (rotationPoint as any).style.transform = `translate(0px, -${rotY - topY + 20}px)`;
            }
        ...
        if (value && shape.type === 'image') {
            const boundingRect: any = window.document.getElementsByClassName('svg_select_boundingRect');
            if (boundingRect) {
    ...
    private onShiftKeyDown = (e: KeyboardEvent): void => {
        if (!e.repeat && e.code.toLowerCase().includes('shift')) {
            ...
            if (this.activeElement) {
                const shape = this.svgShapes[this.activeElement.clientID || 0];
                if (shape && shape?.remember('_selectHandler')?.options?.rotationPoint) {
                    if (this.drawnStates[this.activeElement.clientID || 0]?.shapeType === 'skeleton') {
    ...
    public constructor(model: CanvasModel & Master, controller: CanvasController) {
        ...
        this.drawHandler = new DrawHandlerImpl(
            this.onDrawDone.bind(this) as any,
            this.adoptedContent,
            this.adoptedText,
        ...
        if (reason === UpdateReasons.CONFIG_UPDATED) {
            ...
            if (!image) {
                ...
                if (model.imageIsDeleted) {
                    ...
                    const canvasContext: any = this.background.getContext('2d');
    ...
    private redrawBitmap(): void {
        ...
        const ctx: any = this.bitmap.getContext('2d');
    ...
    private getShapeColorization(state: any, opts: {
        ...
        return {
            ...
            'fill-opacity': !['polyline', 'points', 'skeleton'].includes(shapeType) || parentShapeType === 'skeleton' ? (shapeOpacity || 0) : 0,
        };
    }
    ...
    private updateObjects(states: any[]): void {
        for (const state of states) {
            ...
            if (
                ...
                state.points.some((p: number, id: number): boolean => p !== (drawnState.points ? drawnState.points[id] : 0))
            ) {
                if (state.shapeType === 'mask') {
            ...
            if (
                ...
                for (const attrID of Object.keys(state.attributes)) {
                    if (state.attributes[attrID] !== drawnState.attributes[+attrID]) {
                        if (text) {
                            const span: any = text.node.querySelectorAll<SVGTSpanElement>(`[attrID="${attrID}"]`);
    ...
    private sortObjects(): void {
        ...
        const states = Array.from(this.content.getElementsByClassName('cvat_canvas_shape')).map((state: any): [
            SVGElement,
            number,
        ...
        crosshair.forEach((line: any): void => this.content.append(line));
        const interaction = Array.from(this.content.getElementsByClassName('cvat_interaction_point'));
        interaction.forEach((circle: any): void => this.content.append(circle));
    ...
    private addText(state: any, options: { textContent?: string } = {}): SVG.Text {
        ...
        const content: any = options.textContent || this.configuration.textContent;
    ...
    private addSkeleton(state: any): any {
        ...
        const templateElements: any = Array.from(SVGElement.children()).filter((el: SVG.Element) => el.type === 'circle');
            ...
            if (element.shapeType === 'points') {
                ...
                const circle = skeleton.circle()
                    ...
                    .attr({
                        ...
                        r: this.configuration.controlPointsSize || 0 / this.geometry.scale,
                        'color-rendering': 'optimizeQuality',
                        'shape-rendering': 'geometricprecision',
                        'stroke-width': consts.BASE_STROKE_WIDTH / this.geometry.scale,
        ...
        (skeleton as any).resize = (action: any) => {
            ...
            if (action !== 'stop') {
                (wrappingRect as any).resize(action).on('resizestart', (): void => {
                    ...
                    setTimeout(() => {
                        let { rotation = 0 } = skeleton.transform();
                        // be sure, that rotation in range [0; 360]
                        while (rotation < 0) rotation += 360;
                        rotation %= 360;
                        showText();
