...
export class DrawHandlerImpl implements DrawHandler {
    ...
    private shapeSizeElement: ShapeSizeElement | null = null;
    ...
    private drawPolyshape(): void {
        let size = this.drawData.shapeType === 'cuboid' ? 4 : (this.drawData.numberOfPoints || 4);
    ...
    private drawSkeleton(): void {
        ...
        this.drawInstance
            .on('drawstop', (e: Event): void => {
                ...
                const [xtl, ytl, xbr, ybr] = this.getFinalRectCoordinates(points, true);
                const elements: any[] = [];
                Array.from(this.pointsGroup.node.children).forEach((child: any) => {
    ...
    private pasteShape(): void {
        const moveShape = (shape: SVG.Shape, x: number, y: number): void => {
            const { rotation = 0 } = shape.transform();
            shape.untransform();
            shape.center(x, y);
        ...
        this.drawInstance.off('done').on('done', (e: CustomEvent) => {
            const result = {
                ...
                elements: this.drawData?.initialState.elements.map((element: any) => ({
                    ...
                    points: (() => {
                        const circle: any = this.pointsGroup?.children()
    ...
    private pastePoints(initialPoints: string): void {
        const moveShape = (shape: SVG.PolyLine, group: any, x: number, y: number, scale: number): void => {
    ...
    public configurate(configuration: Configuration): void {
        ...
        this.autobordersEnabled = configuration.autoborders || false;
