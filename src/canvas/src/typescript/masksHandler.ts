...
export class MasksHandlerImpl implements MasksHandler {
    ...
    private onEditDone: (state: any, points: any) => void;
    ...
    private setupBrushMarker(): void {
        if (['brush', 'eraser'].includes(this.tool?.type || '')) {
            const common = {
                ...
                selectable: false,
                opacity: 0.75,
                left: this.latestMousePos.x - ( this.tool?.size || 0 ) / 2,
                top: this.latestMousePos.y - ( this.tool?.size || 0 ) / 2,
                stroke: 'white',
                strokeWidth: 1,
            };
    ...
    public constructor(
        ...
        this.canvas.on('mouse:move', (e: fabric.IEvent<MouseEvent>) => {
            ...
            if (isBrushSizeChanging && ['brush', 'eraser'].includes(tool?.type || '')) {
                ...
                let onUpdateConfiguration: any = null;
                if (this.isDrawing) {
                ...
                if (onUpdateConfiguration) {
                    onUpdateConfiguration({
                        brushTool: {
                            size: Math.trunc(Math.max(1, (this.tool?.size || 0) + xDiff)),
                        },
                    });
                }
            ...
            if (this.brushMarker) {
                this.brushMarker.left = position.x - ( tool?.size || 0 ) / 2;
                this.brushMarker.top = position.y - ( tool?.size || 0 ) / 2;
            ...
            if (isMouseDown && !isBrushSizeChanging && ['brush', 'eraser'].includes(tool?.type || '')) {
                ...
                const shapeProperties = {
                    ...commonProperties,
                    fill: color.toRgba(),
                    left: position.x - ( tool?.size || 0 ) / 2,
                    top: position.y - ( tool?.size || 0 ) / 2 ,
                };
                ...
                if (this.latestMousePos.x !== -1 && this.latestMousePos.y !== -1) {
                    ...
                    if (Math.sqrt(dx ** 2 + dy ** 2) > ( tool?.size || 0 ) / 2) {
                        const line = new fabric.Line([
                            this.latestMousePos.x - ( tool?.size || 0 ) / 2,
                            this.latestMousePos.y - ( tool?.size || 0 ) / 2,
                            position.x - ( tool?.size || 0 ) / 2,
                            position.y - ( tool?.size || 0 ) / 2,
                        ], {
                            ...commonProperties,
                            stroke: color.toRgba(),
    ...
    public configurate(configuration: any): void {
