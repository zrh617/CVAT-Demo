...
import { CanvasMode as Canvas3DMode } from '@/canvas3d-wrapper';
import {
...
export function repeatDrawShapeAsync(): ThunkAction {
    return async (dispatch: ActionCreator<Dispatch>): Promise<void> => {
        ...
        if (activeInteractor && canvasInstance instanceof Canvas) {
            if ((activeInteractor as any).type.includes('tracker')) {
                canvasInstance.interact({
                    enabled: true,
                    shapeType: 'rectangle',
                });
                dispatch(interactWithCanvas(activeInteractor, activeLabelID as any));
                dispatch(switchToolsBlockerState({ buttonVisible: false }));
            } else {
                canvasInstance.interact({
                    enabled: true,
                    shapeType: 'points',
                    ...(activeInteractor as any).params.canvas,
                });
                dispatch(interactWithCanvas(activeInteractor, activeLabelID as any));
            }
