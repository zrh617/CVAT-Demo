...
export class Canvas3dViewImpl implements Canvas3dView, Listener {
    ...
    public notify(model: Canvas3dModel & Master, reason: UpdateReasons): void {
        if (reason === UpdateReasons.IMAGE_CHANGED) {
            ...
            const objectURL = URL.createObjectURL(model.data.image.imageData as any);
            this.clearScene();
            if (this.controller.imageIsDeleted) {
                this.render();
                const container: any = window.document.getElementsByClassName('cvat-canvas-container');
                ...
                const canvasContext: any = overlay.getContext('2d');
                ...
                const overlay: any = window.document.getElementsByClassName('cvat_3d_canvas_deleted_overlay');
                if (overlay) {
                    overlay.remove();
                }
            }
    ...
    private setSelectedChildScale(x: any, y: any, z: any): void {
        [ViewType.TOP, ViewType.SIDE, ViewType.FRONT].forEach((view: ViewType): void => {
            this.model.data.selected[view].children.forEach((element: any): void => {
    ...
    private detachCamera(view: any): void {
    ...
    private rotatePlane(direction: any, view: any): void {
